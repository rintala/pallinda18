...
func main() {
	...
	for n, fn := range Funcs {
		//adding one to waitgroup element for each function in Funcs variable
		wg.Add(1)
		//before := time.Now()
...
//How many CPUs does you program use?
	/* 	- 8 cores by default
		- i.e. will not help efficieny to specify more than 8 virtual ones
	*/

//How much faster is your parallell version?
	/*	- Total time improves by approx. 10ms
		- By 50%, from 20ms => 10ms
	*/
//What concurrency have we introduced?
	/* 	- New goroutines for each pixel up to a limit of 1000 goroutines - trial & error
		- Also parallellized the 7 different images in separate goroutines
	*/
